...
__global__ void race(int *A, int N){
    ...
    if(local_id < 64)
        __syncthreads();                // only some threads reach this barrier, so it is an incorrect synchronization
    if (local_id == 0)
        A[blockIdx.x * blockDim.x] = shared_data[128];
}
